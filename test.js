...
test('horizontal line cutting 6 edges', function (t) {
	let out = splitPolygon(poly, [0,4], [1,0]);
	t.ok(isEqual(out, [ 
  [ [ 1, 1 ], [ 1.75, 4 ], [ 1.6, 4 ] ],
  [ [ 1.75, 4 ],
    [ 2, 5 ],
...
test('splitting line intersects vertex 3', function (t) {
	let out = splitPolygon(poly, [0,3], [1,0]);
	//FIXME duplicate [3,3]
/*	t.ok(isEqual(out, [ 
  [ [ 1, 1 ], [ 1.5, 3 ], [ 1.4, 3 ] ],
  [ [ 1.5, 3 ],
...
test('splitting line intersects vertices 2 and 4', function (t) {
	let out = splitPolygon(poly, [0,5], [1,0]);
	//FIXME duplicates
...
test('splitting line contains edge', function (t) {
	let out = splitPolygon(poly, [0,6], [1,0]);
	//FIXME
/*	t.ok(isEqual(out, [ 
  [ [ 1, 1 ],
    [ 2, 5 ],
...
test('arbitrary splitting line direction', function (t) {
	let out = splitPolygon(poly, [0,-1], [1,1]);
